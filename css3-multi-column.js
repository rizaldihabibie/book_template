...
// Adapted by Peter Craddock for multi-page implementation. Search for "VALUE TO CHANGE" for values to change!
// For further information, see http://www.arpia.be/


function CSS3MultiColumn() {
	//alert('Development Version');
	var cssCache = new Object();
	var splitableTags = new Array('P','DIV', 'SPAN', 'TABLE', 'TBODY', 'TR', 'TD', 'BLOCKQUOTE','ADDRESS','PRE', 'EM', 'I', 'STRONG', 'B', 'CITE', 'OL', 'UL', 'LI' );
		...
		if (s.href && typeof XMLHttpRequest!='undefined') {	
			...
			xmlhttp.onreadystatechange = function() {
				if(xmlhttp.readyState == 4) {
					if(typeof xmlhttp.status == 'undefined' || !xmlhttp.status || xmlhttp.status == 200 || xmlhttp.status == 304 ) {
	...
	function processElement(affectedElement, column_count, column_width, column_gap, column_rule ) {
		...
		if(affectedElement.offsetWidth && affectedElement.offsetWidth != 0) {			
			var padding;
			...
			width = (affectedElement.offsetWidth - padding).toString() + "px";
		}
		...
		var availableWidth = 16400; // !! VALUE TO CHANGE
		...
		var newHeight = 430; // !! VALUE TO CHANGE
		if(!wrapper.id) wrapper.id = "columnified";
		...
		for(var i=1; i < column_count && elem; i++) {
			...
			findSplitPoint(elem, 430, wrapper); // !! VALUE TO CHANGE
